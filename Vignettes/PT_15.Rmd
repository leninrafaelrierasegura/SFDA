...
for (j in 1:length(distance)) {
  print(j)
  GROUPSOUTSIDE[[j]] = list()
  for (i in 1:nrow(points)) {
    rowi = points[i, ]
    GROUPSOUTSIDE[[j]][[i]] <- which(as.vector(distmatrixlist[[rowi$.group]][rowi$indexingroup,]) > distance[j])
  }
}
save(GROUPSOUTSIDE, file = here("Models_output/GROUPSOUTSIDE_for_window_case.RData"))
```


```{r, fig.height = 8}
mse.vector <- rep(0,length(distance))
for (j in 1:length(distance)) {
  print(j)
  localpred = rep(NA, nrow(points))
  for (i in 1:nrow(points)) {
    row_i <- points[i,]
    dd <- points |> 
      filter(.group == row_i$.group) |> 
      dplyr::select(index) |> 
      cbind(vec_i = distmatrixlist[[row_i$.group]][row_i$indexingroup,])
    dd <- dd[-GROUPS[[j]][[i]],] |> arrange(vec_i)
    localpred[i] = points$speed[dd[1,1]]
  }
  mse.vector[j] = mean((localpred - points$speed)^2)
}

#save(distance, mse.vector, file = here("Models_output/mse.vector.RData"))
...
# Plot MSE
plot(distance, mse.vector, main = "MSE", type = "l", ylab = "MSE", xlab = "distance in km", col = "black")
points(distance, mse.vector)
```
```{r}
plot(density(log(points$speed)))
```
