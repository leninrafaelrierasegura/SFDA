...
```{r}
range(newdata_on_graph$.distance_to_graph)
```
...
auxDAG = sf_graph$get_data() 
auxDAG |> dim()
```

# DATA AFTER BEING ADDED TO THE GRAPH BUT WITHOUT USING PLOT METHOD

```{r}
(sf_graph$plot(vertex_size = 0) + 
  geom_point(data = auxDAG, aes(x = .coord_x, y = .coord_y, color = speed), size = 1) +
  scale_color_viridis_c(option = "D")) |> ggplotly()
```
...
# PLOTTING SPEEDLIMITS BEFORE ADDING IT TO THE GRAPH
