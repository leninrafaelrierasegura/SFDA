...
## SIMULATE THE STATIONARY RESPONSE

```{r}
speed_sim <- 1 + 2*data_final$SpeedLimit + as.vector(A.proj %*% u_stat + sigma.e * rnorm(nrow(data_final)))
```

## CREATE A NEW DATASET (THE ONLY THING THAT CHANGES IS THE SIMULATED STATIONARY RESPONSE)
...
## ADD THE DATA WITH SIMULATED RESPONSE (USING STATIONARY FIELD)
...
for (j in 1:length(distance)) {
  ...
  mse.nonstat[j] <- mean((cv.nonstat$mean - data_sim_stat$speed)^2)
...
plot(distance, mse.stat, main = "MSE", ylim = c(min(mse.nonstat, mse.stat), max(mse.nonstat, mse.stat)),
     type = "l", ylab = "MSE", xlab = "distance in m", col = "red", lty = 2)
lines(distance, mse.nonstat, col = "blue", lty = 3)
legend("bottomright", legend = c("Stationary", "Non-stationary"), col = c("red", "blue"), lty = 1)

# Plot log-score
plot(distance, -ls.stat, main = "log-score", ylim = c(min(-ls.nonstat, -ls.stat), max(-ls.nonstat, -ls.stat)),
     type = "l", ylab = "log-score", xlab = "distance in m", col = "red", lty = 2)
lines(distance, -ls.nonstat, col = "blue", lty = 3)
legend("bottomright", legend = c("Stationary", "Non-stationary"), col = c("red", "blue"), lty = 1)
