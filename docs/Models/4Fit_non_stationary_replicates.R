...
B.sigma = cbind(0, 1, 0, 
                mesh$SpeedLimit, 0,
                mesh$density_per_hour_normalized, 0, 
                mesh$bus, 0,
                mesh$signal, 0,
                mesh$stop, 0,
                mesh$crossing, 0) 
...
B.range = cbind(0, 0, 1, 
                0, mesh$SpeedLimit,
                0, mesh$density_per_hour_normalized,
                0, mesh$bus,
                0, mesh$signal,
                0, mesh$stop,
                0, mesh$crossing) 
...
data_rspde_bru_nsWM1 <- graph_data_rspde(rspde_model_nonstatWM1, repl = ".all", loc_name = "loc")
...
cmp_nonstatWM1 = speed ~ -1 +
  ...
  stop +
  crossing +
  field(loc, model = rspde_model_nonstatWM1, replicate = day)
...
data_rspde_bru_nsWM2 <- graph_data_rspde(rspde_model_nonstatWM2, repl = ".all", loc_name = "loc")
...
cmp_nonstatWM2 = speed ~ -1 +
  ...
  stop +
  crossing +
  field(loc, model = rspde_model_nonstatWM2, replicate = day)
...
save.image("nonstationary_with_replicates_gaussian.RData")
