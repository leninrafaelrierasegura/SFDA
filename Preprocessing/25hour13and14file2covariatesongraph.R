...
load("Data_files/data_day7142128_hour13and14_with_no_consecutive_zeros_graphsubset_graph_processed.RData")
load("Data_files/stops_data_on_graphsubset.RData")

to_remove = data_on_graph %>% filter(speed == 0, .distance_to_graph > 0.005) # removing zero speed observations that are 5m or more away from the graph
data_on_graph = setdiff(data_on_graph, to_remove) %>% filter(.distance_to_graph <= 0.007)
...
save(data_on_graph_with_covariates, file = "Data_files/data_on_graph_with_covariates_no_consecutive_zerosgraphsubset_hour13and14.RData")
